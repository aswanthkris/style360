...
exports.verifyOtp = (req, res) => {
  ...
  otp
    .veriOtp(otpObject.otp, req.session.mobileNumber)
    .then((verify) => {
      if (verify) {
        ...
        .then((wishlist) => {
          Category.find()
            .then((category) => {
              ...
              res.render("user/home", { category, cart, wishlist });
            })
            .catch((err) => {
              console.log(err);
            });
                          ...
                          .then((cart) => {
                            ...
                            if (cart) {
                              if (user.address.length) {
                                ...
                                res.redirect(
                                  "/cart/checkout/shipping/add-new-address?userAddress=false"
                                );
                              }
...
exports.adminProducts = (req, res) => {
  Product.find()
    ...
    .then((result) => {
      let num = 1;
      if (result) res.render("admin/adminProducts", { result, num });
    })
    .catch((err) => console.log(err));
};
...
exports.viewOrders = (req, res) => {
  ...
  Order.findOne({ _id: ObjectId(orderId) }).then((order) => {
...
exports.orderAccept = (req, res) => {
  ...
  Order.updateOne(
    { _id: ObjectId(orderId), "items.itemId": itemId },
...
exports.orderCancel = (req, res) => {
  ...
  Order.updateOne(
    { _id: ObjectId(orderId), "items.itemId": itemId },
    { $set: { "items.$.orderStatus": "Vendor Cancelled" } }
  )
    .then(() => {
      res.redirect(`/view-orders?id=${orderId}`);
    })
...
exports.paypal = (req, res) => {
  let billAmount = Order.findOne({ owner: req.session.userId }).then(
    (order) => {
  ...
  billAmount.then((bill) => {
    ...
    Paypal.configure({
      ...
      client_id:
        "AW-mmXtzC_WMOCQJD0ndbcTy87TuyA7ElnCuPLbWOl34olDBKsz6pFmt8z9JN3Uv_YQKugWVx9nVDYR-",
      // please provide your client id here
      client_secret:
        "EGHQ38LAdNXKnz-yg34PIoiKq0Gn0LtJk1W706DF7HtRpfRhehqYnqSyO_PDpQ-gPPLYn3f1HlRUmqnG", // provide your client secret here
    });
...
exports.userOrderCancel = (req, res) => {
  ...
  Order.updateOne(
    { _id: ObjectId(orderId), "items.itemId": itemId },
    { $set: { "items.$.orderStatus": "User Cancelled" } }
  )
    .then(() => {
    ...
    .then((orders) => {
      ...
      orders.forEach((order) => {
        if (order.orderDate.getMonth() + 1 <= 3) {
          ...
          order.orderDate.getMonth() + 1 > 3 &&
          order.orderDate.getMonth() + 1 <= 6
        ) {
          quarters[1].push(order);
        } else if (
          order.orderDate.getMonth() + 1 > 6 &&
          order.orderDate.getMonth() + 1 <= 9
        ) {
          quarters[2].push(order);
        } else if (
          order.orderDate.getMonth() + 1 > 9 &&
          order.orderDate.getMonth() + 1 <= 12
        ) {
      ...
      res.json({
        salesOfTheYear: monthlySalesTurnover,
        quarterlySales: quarterlySalesTurnover,
